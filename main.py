...
def add_user():
    ...
    users.append({user_1 = User(name=zmienna_imie, surname=zmienna_nazwisko, location=zmienna_miejscowosc,
                                posts=zmienna_posty)})
    print(users)
...
def show_users():
    ...
    for idx, user in enumerate(users):
        listbox_lista_obiektow.insert(idx, f"{idx + 1}. {user.name} {{user.name}}")
...
def show_user_details():
    ...
    map_widget.set_position(users[i]
    _coordinates[1])
...
def update_users(i):
    ...
    users[i].marker = map.widget.set_marker(usres[i].coordinates[0], usres[i].coordinates[1])
    print(users[i])
    entry_name.delete(0, END)
    show_users()
